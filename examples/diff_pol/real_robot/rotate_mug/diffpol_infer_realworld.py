...
def infer(nets, data_utils, rpc, cfg, execute_steps=8, obs_horizon=2, ctr_grip=False, video_name='', no_crop=True,
          num_diffusion_iters=10, crop_percent=0.2, add_info='', block=True, save_folder=''):
    step = 0
    # resize Height * Width
    if no_crop:
    ...
    while True:
        ...
        for id, action in enumerate(actions[1:1 + execute_steps]):
            ...
            if block:
                rpc.goto_cartesian_pose_blocking(action[:3], action[3:7] / np.linalg.norm(action[3:7]), GENERATE_TRAJECTORY)
            else:
                rpc.goto_cartesian_pose_nonblocking(action[:3], action[3:7] / np.linalg.norm(action[3:7]),
                                                    GENERATE_TRAJECTORY)
                # rpc.goto_cartesian_pose_nonblocking(action[:3], action[3:7],
                #                                     GENERATE_TRAJECTORY)
            ...
            fix_img, gripper_image = get_image(rs_recorder=rs_recorder, transform_cam=transform_cam,
                                               ...
                                               grip_img_list=grip_img_deque)
            fix_img_deque.append(fix_img)
            grip_img_deque.append(gripper_image)
            # fix_img_list.append(fix_img.cpu().numpy())
            step += 1
            save_array_safe(fix_img_list, save_folder + '/img_' + video_name)
            save_array_safe(real_state_list, save_folder + '/state_' + video_name)
            save_array_safe(predict_action_list, save_folder + '/action_' + video_name)
            save_array_safe(infer_time_list, save_folder + '/time_' + video_name)
            # np.save('DDIM/' + video_name, fix_img_list)
            # np.save('DDIM' + '/record/state' + video_name, real_state_list)
...
if __name__ == '__main__':
    ...
    nets.load_state_dict(torch.load(cfg.diffusion_model + add_info + '/' + cfg.diffusion_model + add_info + '.pt'))
    nets.to(device)
    ...
    save_folder = 'DDIM_harddata_/show_demo_step' + str(num_diffusion_iters)
    if not os.path.exists(save_folder):
        os.mkdir(save_folder)

    infer(nets=nets, data_utils=data_utils, rpc=rpc, cfg=cfg, execute_steps=execute_steps, ctr_grip=True,
          video_name='0' + '_execute' + str(execute_steps) + '_nonblock' + '.npy', no_crop=False, crop_percent=0.2,
          num_diffusion_iters=num_diffusion_iters, add_info=add_info, block=False, save_folder='DDIM_harddata_/step' + str(num_diffusion_iters))
