...
def infer_whole_traj(data_loadar, nets, data_utils, noise_scheduler, action_horizon=8, save_video=False, no_crop=True,
    ...
    for i, batch in tqdm(enumerate(data_loadar)):
        if i % action_horizon == 0:
            ...
            if i == 0:
                gen_state_list = naction[:action_horizon].cpu().numpy()
                demo_state_list = batch['future_pos_quat'][0][:action_horizon].cpu().numpy()
                demo_grip_list = batch['smooth_future_gripper'][0][:action_horizon].cpu().numpy()
            else:
                gen_state_list = np.concatenate([gen_state_list, naction[:action_horizon].cpu().numpy()])
                demo_state_list = np.concatenate(
                    [demo_state_list, batch['future_pos_quat'][0][:action_horizon].cpu().numpy()])
...
if __name__ == '__main__':
    ...
    infer_whole_traj(data_loadar=val_loader, nets=nets, data_utils=data_utils, noise_scheduler=noise_scheduler,
                     action_horizon=8, save_video=False, no_crop=False, num_diffusion_iters=10)
