...
class PosDataUtils:
    def __init__(self, cfg, nets, device=torch.device('cuda')):
        self.cfg = cfg
        self.device = device
        self.pos_max = torch.tensor([0.7, 0.3, 0.4]).to(self.device)
        self.pos_min = torch.tensor([0.2, -0.1, 0.]).to(self.device)
        self.dim = 3 + 1
    ...
    def normalize_pos(self, pos):
        return 2 * (pos - self.pos_min.to(self.device)) / (
                self.pos_max.to(self.device) - self.pos_min.to(self.device)) - 1

    def denormalize_pos(self, norm_pos):
        return (norm_pos + 1) / 2 * (
                self.pos_max.to(self.device) - self.pos_min.to(self.device)) + self.pos_min.to(
            self.device)
    ...
    def unpack_predictions_reference_conditioning_samples(self, batch: torch.Tensor):
        ...
        x1_pos = batch['future_pos_quat'][..., :3].to(self.device)
        x1_gripper = batch['future_gripper'].to(self.device)
        if self.normalize:
            x1_pos = self.normalize_pos(x1_pos)
            x1_gripper = self.normalize_grip(x1_gripper)
        x1 = torch.cat((x1_pos, x1_gripper), dim=2)
        ...
        xref_pos_pre = batch['previous_pos_quat'].squeeze(dim=1)[..., :3].to(self.device)
        xref_pos_cur = batch['future_pos_quat'][:, 0, :3].to(self.device)
        if self.normalize:
            xref_pos_pre = self.normalize_pos(xref_pos_pre)
            xref_pos_cur = self.normalize_pos(xref_pos_cur)
        xref_pos = torch.cat((xref_pos_pre, xref_pos_cur), dim=1)
        ...
        xref = torch.cat((img_scalar, xref_pos, xref_gripper), dim=1)
        return x1, xref


def train(nets, cfg, train_loader, val_loader, num_epochs=300):
    ema = EMAModel(nets)
    data_utils = PosDataUtils(cfg, nets=nets, device=device)
        ...
        for epoch_idx in tglobal:
            ...
            if epoch_idx % 5 == 0:
                ...
                if cur_best_val_loss is None:
                    ...
                    check_point(nets, cfg.diffusion_model + '_pos_' + '.pt')
                    np.savetxt(cfg.diffusion_model + '_pos_' + 'best_val_epoch.txt', [epoch_idx])
                else:
                    if cur_best_val_loss > np.mean(val_loss_list):
                        check_point(nets, cfg.diffusion_model + '_pos_' + '.pt')
                        np.savetxt(cfg.diffusion_model + '_pos_' + 'best_val_epoch.txt', [epoch_idx])
            np.save(cfg.diffusion_model + '_pos_' + 'epoch_loss_list.npy', epoch_total_loss)
            np.save(cfg.diffusion_model + '_pos_' + 'val_apoch_loss_list.npy', total_val_loss)
...
if __name__ == '__main__':
    ...
    num_epochs = 300
    action_dim = 3 + 1
    vision_feature_dim = 512 + action_dim
