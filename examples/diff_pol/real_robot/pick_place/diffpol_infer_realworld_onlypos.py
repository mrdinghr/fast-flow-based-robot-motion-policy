...
from diffpol_cup_on_plate_onlypos import PosDataUtils
import tempfile
...
def get_ref(state_list, grip_state_list, fix_img_list, grip_img_list, data_utils):
    ...
    xref_pos = torch.tensor(state_list)[..., :3].to(device)
    xref_pos = data_utils.normalize_pos(xref_pos).reshape((1, 6))
    print('normalize ref', xref_pos)

    xref_gripper = torch.tensor(grip_state_list).unsqueeze(0).to(device)
    xref_gripper = data_utils.normalize_grip(xref_gripper)
    return torch.cat((img_scalar, xref_pos, xref_gripper), axis=1).float()
...
def infer(nets, data_utils, rpc, cfg, execute_steps=8, obs_horizon=2, ctr_grip=False, video_name='', no_crop=True,
    ...
    while True:
        ...
        if cfg.normalize_pos_quat:
            actions[..., :-1] = data_utils.denormalize_pos(actions[..., :-1])
        ...
        for id, action in enumerate(actions[1:1 + execute_steps]):
            ...
            # rpc.goto_cartesian_pose_blocking(action, np.array([0., 1., 0., 1.]), GENERATE_TRAJECTORY)
            if ctr_grip:
...
if __name__ == '__main__':
    ...
    nets.load_state_dict(torch.load('DDIM_onlypos/DDIM_pos_.pt'))
    nets.to(device)
    data_utils = PosDataUtils(cfg=cfg, nets=nets)
