...
from diffplo_robomimic_train_img import DiffpolRobomimicDataUtils
...
def get_ref(obs_deque):
    ...
    pre_xref_img_scalar = data_utils.image_to_features(
        crop_transform(torch.from_numpy(pre_xref_img).float().to(device).unsqueeze(0).unsqueeze(0)))
    cur_xref_img_scalar = data_utils.image_to_features(
        crop_transform(torch.from_numpy(cur_xre_img).float().to(device).unsqueeze(0).unsqueeze(0)))
    ...
    pre_xref_grip_img_scalar = data_utils.grip_image_to_features(
        crop_transform(torch.from_numpy(pre_xref_grip_img).float().to(device).unsqueeze(0).unsqueeze(0)))
    cur_xref_grip_img_scalar = data_utils.grip_image_to_features(
        crop_transform(torch.from_numpy(cur_xref_grip_img).float().to(device).unsqueeze(0).unsqueeze(0)))
...
if __name__ == '__main__':
    ...
    if cfg.task == 'tool_hang':
        ...
        max_steps = 500
    for epoch_id in range(1, save_num + 1):
        model_folder = cfg.task
        ...
        for num_diffusion_steps in [3]:
            ...
            for seed in tqdm(range(1, 51)):
                np.random.seed(seed * 6)
                torch.manual_seed(seed * 6)
                success, trans_traj, rotation_traj = infer(nets=nets, num_diffusion_steps=num_diffusion_steps,
