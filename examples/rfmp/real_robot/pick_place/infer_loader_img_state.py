...
from dummy_robot_arm1 import get_loaders, get_debug_loaders
...
def infer_whole_traj(data_loadar, model, execute_horizon=8, save_video=False, no_crop=True, ode_steps=11):
    ...
    for i, batch in tqdm(enumerate(data_loadar)):
        if i % execute_horizon == 0:
            ...
            if i == 0:
                ...
                demo_state_list = batch['future_pos_quat'][0][:execute_horizon].cpu().numpy()
                demo_grip_list = batch['smooth_future_gripper'][0][:execute_horizon].cpu().numpy()
            else:
                gen_state_list = np.concatenate([gen_state_list, actions[:execute_horizon].cpu().numpy()])
                demo_state_list = np.concatenate([demo_state_list, batch['future_pos_quat'][0][:execute_horizon].cpu().numpy()])
                demo_grip_list = np.concatenate([demo_grip_list, batch['smooth_future_gripper'][0][:execute_horizon].cpu().numpy()])
    ...
    plt.show()
    print(np.array(infer_time_list).min())
    print(infer_time_list)
