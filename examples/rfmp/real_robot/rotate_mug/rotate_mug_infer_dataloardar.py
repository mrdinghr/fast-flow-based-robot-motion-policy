...
def infer_whole_traj(data_loadar, model, execute_horizon=8, save_video=False, no_crop=True, guided_flow=False):
    ...
    for i, batch in tqdm(enumerate(data_loadar)):
        if i % execute_horizon == 0:
            ...
            actions = model.sample_all(1, model.device, xref=xref, xcond=xcond, different_sample=True, ode_steps=11,
...
if __name__ == '__main__':
    ...
    add_info = '_vision_target_crop02_st250_old'
    ...
    infer_whole_traj(val_loader, model, execute_horizon=8, save_video=False, guided_flow=True)
