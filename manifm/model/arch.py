...
class ProjectToTangent(nn.Module):
    ...
    def __init__(self, vecfield, manifold, metric_normalize, dim=None):
        super().__init__()
        self.vecfield = vecfield
    ...
    def forward(self, t, x):
        if self.dim is not None:
            xm = x[:, :self.dim]  # In the case of conditioning, we assume that the first dimensions are on the manifold
        else:
            xm = x
        if isinstance(self.manifold, Mesh):  # TODO use xm where necessary!
        ...
        if isinstance(self.manifold, SPD):
            ...
            v = self.vecfield(t, x)
            v = self.manifold.proju(xm, v)
        else:
            xm = self.manifold.projx(xm)
            v = self.vecfield(t, x)
            v = self.manifold.proju(xm, v)
