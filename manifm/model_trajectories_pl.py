...
from manifm.model.uNet import ConditionalUnet1D
...
class ManifoldTrajectoriesFMLitModule(ManifoldFMLitModule):
    ...
    @torch.no_grad()
    def sample_all(self, n_samples, device, xref, xcond, x0=None, shift_mean=False, different_sample=False):
        if x0 is None:
            # Sample from base distribution.
            # x0 = (
    ...
    def compute_exact_loglikelihood(
        ...
        with torch.inference_mode(mode=False):
            ...
            wrapper_vecfield = self.wrapper_red_cond(self.vecfield, xref, xcond, model_type=self.model_type)

            v = None
            if div_mode == "rademacher":
                v = torch.randint(low=0, high=2, size=xbatch.shape).to(xbatch) * 2 - 1
            ...
            with torch.no_grad():
                if not eval_projx and not local_coords:
                    ...
                    state0 = projx_integrator_return_last(
                        ...
                        odefunc,
                        state1,
                        t=torch.linspace(t1, 0, num_steps + 1).to(xbatch),
                        method="euler",
                        projx=eval_projx,
                        local_coords=local_coords,
                        pbar=True,
